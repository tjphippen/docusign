...
class Docusign
{
    ...
    {
        $request = $this->client->get('users', $this->clientSettings);
    ...
    public function getUser($userId, $additional_info = false)
    {
        ...
        $request = $this->client->get('users/' . $userId . '?additional_info=' . $additional_info, $this->clientSettings);
    ...
    public function getEnvelopes($envelopeIds)
    {
        ...
        $request = $this->client->put('envelopes/status', array_merge($this->clientSettings, ['json' => $envelopes, 'query' => ['envelope_ids' => 'request_body']]));
        return $envelopes = $this->rawJson($request);
    }

    public function getEnvelope($envelopeId)
    {
        $request = $this->client->get('envelopes/' . $envelopeId, $this->clientSettings);
        return $envelope = $this->rawJson($request);
    }

    public function getEnvelopePdf($envelopeId)
    {
        $request = $this->client->get('envelopes/' . $envelopeId . '/documents/combined?certificate=true', $this->clientSettings);
    ...
    public function getEnvelopeRecipients($envelopeId, $include_tabs = false)
    {
        ...
        $request = $this->client->get('envelopes/' . $envelopeId . '/recipients?include_tabs=' . $include_tabs, $this->clientSettings);
        return $recipients = $this->rawJson($request);
    }

    public function getEnvelopeTabs($envelopeId, $recipientId)
    {
        $request = $this->client->get('envelopes/' . $envelopeId . '/recipients/' . $recipientId . '/tabs', $this->clientSettings);
        return $tabs = $this->rawJson($request);
    }

    public function createEnvelope($data) {
        $request = $this->client->post('envelopes/', array_merge($this->clientSettings, ['json' => $data]));
        return $envelope = $this->rawJson($request);
    }

    public function updateEnvelope($envelopeId, $data)
    {
        $request = $this->client->put('envelopes/' . $envelopeId, array_merge($this->clientSettings, ['json' => $data]));
        return $envelope = $this->rawJson($request);
    }

    public function updateRecipientTabs($envelopeId, $recipientId, $tabs)
    {
        $request = $this->client->put('envelopes/' . $envelopeId . '/recipients/' . $recipientId . '/tabs', array_merge($this->clientSettings, ['json' => $tabs]));
        return $tabs = $this->rawJson($request);
    }

    public function deleteEnvelope($envelopeId) {
        $data = array('envelopeIds' => array($envelopeId));
        $request = $this->client->put('folders/recyclebin', array_merge($this->clientSettings, ['json' => $data]));
        return $deleted = $this->rawJson($request);
    }

    public function getTemplates($options = null)
    {
        $request = $this->client->get('templates', array_merge($this->clientSettings, ['query' => $options]));
    ...
    {
        $request = $this->client->get('templates/' . $templateId, $this->clientSettings);
        return $template = $this->rawJson($request);
    }

    public function getEnvelopeTemplates($envelopeId)
    {
        $request = $this->client->get('envelopes/' . $envelopeId . '/templates', $this->clientSettings);
        $templates = $this->rawJson($request);
    ...
    public function getFolders($templates = false)
    {
        ...
        $request = $this->client->get('folders/?template=' . $templates, $this->clientSettings);
    ...
    {
        $request = $this->client->get('folders/' . $folderId, array_merge($this->clientSettings, ['query' => $options]));
    ...
    {
        $request = $this->client->get('envelopes/' . $envelopeId . '/custom_fields', $this->clientSettings);
        return $custom_fields = $this->rawJson($request);
    }

    public function createRecipientView($envelopeId, $data)
    {
        $request = $this->client->post('envelopes/' . $envelopeId . '/views/recipient', array_merge($this->clientSettings, ['json' => $data]));
